function progressbar(fractiondone, position, images)
% Description:
...
if isempty(progfig)
    ...
    if img
        ...
        imgwidth = size(R,1)/1000;
        imgheight = size(R,2)/1000;
    end
...
if (fractiondone == 0)
    titlebarstr = 'CV match 0% complete';
else
    runtime = etime(clock,starttime);
    timeleft = runtime/fractiondone - runtime;
    timeleftstr = sec2timestr(timeleft);
    titlebarstr = sprintf('Running CV template match: ~%2d%%    %s remaining',percentdone,timeleftstr);
end
